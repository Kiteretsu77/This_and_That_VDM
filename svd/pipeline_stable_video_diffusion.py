...
class StableVideoDiffusionPipeline(DiffusionPipeline):
    ...
    def encode_clip(self, image, prompt, use_text, text_encoder, device, num_videos_per_prompt, do_classifier_free_guidance):
        # Encode image and text prompt by the clip
    ...
    def __call__(
        ...
        # 3. Encode input image by CLIP
        encoder_hidden_states = self.encode_clip(image, prompt, use_text, text_encoder, device, num_videos_per_prompt, do_classifier_free_guidance)
        ...
        # cv2.imwrite("no_noise.png", cv2.cvtColor(tensor2np(image), cv2.COLOR_BGR2RGB))
        noise = randn_tensor(image.shape, generator=generator, device=image.device, dtype=image.dtype)
        image = image + noise_aug_strength * noise
        # cv2.imwrite("noise.png", cv2.cvtColor(tensor2np(image), cv2.COLOR_BGR2RGB))
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet(
                    latent_model_input,     # [batch, frames, 4*2, height, width]
                    t,
                    encoder_hidden_states = encoder_hidden_states,
                    added_time_ids = added_time_ids,
                    return_dict = False,
                )[0]        # encoder_hidden_states is used for cross attention metioned in the paper
